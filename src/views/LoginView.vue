...
const { loading, error, isAdmin } = storeToRefs(authStore);
...
async function handleLogin() {
  ...
  if (!authStore.error) {
    // اگر ادمین است بفرست به داشبورد ادمین، وگرنه به صفحه رقابت
    if (isAdmin.value) {
...
// اگر قبلاً لاگین است، مستقیم هدایت
watch(
  () => authStore.isAuthenticated,
  (val) => {
    if (val) {
      if (isAdmin.value) router.push({ name: "admin-dashboard" });
      else router.push({ name: "competition" });
    }
  },
  { immediate: true }
);
